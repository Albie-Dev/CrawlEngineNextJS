...
export async function GET() {
  ...
  const fakeVideos = (count: number, channelBase: string, baseViews: number, baseEng: number, recent: boolean) => {
    return Array.from({ length: count }).map((_, i) => ({
      ...
      views: Math.round(baseViews + (Math.random() * baseViews * 0.5)),
  ...
  const t6 = fakeTopic("bottom-left-2", "Lãi tiết kiệm an toàn", 12, 10, 15000, 0.02, -25, 180000, "Thấp", false); // Red, older (60+ days)

  // Bottom Right: Low Perf, High Comp
  const t7 = fakeTopic("bottom-right-1", "Chứng khoán hàng ngày", 150, 40, 10000, 0.01, -30, 1500000, "Thấp", true); // Red
  const t8 = fakeTopic("bottom-right-2", "Dự báo giá vàng", 120, 35, 25000, 0.05, -5, 3000000, "Thấp", true); // Yellow

  // Additional varied topics
  const t9 = fakeTopic("mid-1", "Cổ phiếu ngành tech", 30, 12, 80000, 0.15, 18, 2400000, "Trung bình", true); // Green
  ...
  try {
    ...
    return NextResponse.json({ ok: true, message: "Đã tạo dữ liệu giả lập thành công!" });
  } catch (error: any) {
