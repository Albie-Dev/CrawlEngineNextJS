  ...
  return (
    <>
      ...
      {jobId && status?.state === "running" && (
        <div className="fixed bottom-6 right-6 z-50 flex max-h-[80vh] w-[360px] flex-col rounded-lg border border-kolia-line dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl">
          {/* Header */}
          <div className="flex items-center gap-3 border-b border-kolia-line dark:border-slate-800 px-4 py-3">
            <Loader2 className="h-5 w-5 animate-spin text-kolia-green" />
            <p className="text-sm font-bold text-kolia-ink dark:text-slate-100 flex-1">Đang đồng bộ dữ liệu...</p>
            <button
              type="button"
          ...
          <div className="flex flex-col gap-3 px-4 py-3">
              ...
              return (
                <div key={plat} className="flex flex-col gap-1">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-bold capitalize text-kolia-ink dark:text-slate-100">{plat}</p>
                    {pData.phase === "done" ? (
            ...
            {platforms.length === 0 && (
              <p className="text-xs text-slate-500 dark:text-slate-400">Đang khởi tạo...</p>
            )}
          </div>

          {/* Logs (expandable) */}
          {expanded && status.logs && status.logs.length > 0 && (
            <div className="max-h-[300px] overflow-y-auto border-t border-kolia-line dark:border-slate-800 px-4 py-2">
              <p className="mb-1 text-[10px] font-bold uppercase tracking-wider text-slate-400">
                Log chi tiết
              </p>
              <div className="space-y-1">
                {status.logs.map((log, i) => (
                  <p key={i} className="text-[11px] leading-5 text-slate-600 dark:text-slate-400">
                    {log}
                  </p>
                ))}
              </div>
      ...
      {showPopup && status && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-md rounded-xl border border-kolia-line dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-2xl">
            <div className="flex flex-col items-center text-center">
              ...
              <h2 className="mt-4 text-xl font-bold text-kolia-ink dark:text-slate-100">
                {status.state === "completed" ? "Đồng bộ hoàn tất!" : "Đồng bộ thất bại"}
              </h2>

              <div className="mt-2 w-full text-sm text-slate-600 dark:text-slate-400">
                {status.state === "completed" ? (
                  <>
                    <p>
                    ...
                    <div className="mt-4 flex justify-center gap-4 text-xs">
                      <div className="rounded border border-kolia-line dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2">
                        <p className="font-bold text-kolia-ink dark:text-slate-100">
                          {status.result?.createdPosts || 0}
                        </p>
                        <p className="text-slate-500 dark:text-slate-400">Bài mới</p>
                      </div>
                      <div className="rounded border border-kolia-line dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2">
                        <p className="font-bold text-kolia-ink dark:text-slate-100">
                      ...
                      <div className="rounded border border-kolia-line dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2">
                        ...
                        <p className="text-slate-500 dark:text-slate-400">Thời gian</p>
                      </div>
                    </div>
                  </>
                ) : (
