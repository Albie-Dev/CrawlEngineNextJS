...
function TranscriptTabContent({ transcript }: { transcript: string }) {
  ...
  if (isJson) {
    return (
      <div className="rounded-lg border border-slate-150 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 p-5 text-xs text-slate-600 dark:text-slate-400 font-mono whitespace-pre-wrap h-[400px] overflow-y-auto custom-scrollbar">
        {transcript}
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 sticky top-0 bg-white dark:bg-slate-900/80 backdrop-blur pb-2 z-10 border-b border-slate-150 dark:border-slate-800 pt-2">
        <h3 className="font-bold text-base text-kolia-ink dark:text-slate-100">Subtitles</h3>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 rounded border border-slate-200 dark:border-slate-800 px-2 py-1 text-[10px] font-semibold text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800 hover:text-slate-700 dark:text-slate-300 transition"
          ...
          <div
            className={`flex items-center gap-1.5 transition-colors ${
              isBilingual ? "text-violet-600" : "text-slate-500 dark:text-slate-400"
            }`}
          >
            <svg
...
function HighlightsTabContent({
  ...
  return (
    <div className="space-y-6">
      {summary && (
        <div className="text-[13px] leading-relaxed text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 p-4 rounded-lg border border-slate-150 dark:border-slate-800 shadow-2xs">
          {summary}
        </div>
      )}
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-base text-kolia-ink dark:text-slate-100">Highlights</h3>
          <div className="flex items-center gap-2">
            <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Expand all</span>
            <button
              onClick={toggleAll}
              className={`relative inline-flex h-4 w-7 items-center rounded-full transition-colors focus:outline-none ${
          ...
          {highlights.map((hl, i) => {
            ...
            return (
              <li key={i} className="relative flex gap-4 pt-1 pb-5 last:pb-0">
                <div className="w-[60px] shrink-0 pt-0.5 relative z-10 bg-white dark:bg-slate-900">
                  <span className="text-[12px] font-medium text-kolia-purple">{hl.timestamp || "00:00"}</span>
                </div>
                <div className="flex-1 min-w-0 pt-0.5">
                  <p className="text-[13px] text-slate-800 dark:text-slate-200 font-semibold leading-relaxed mb-1.5">{hl.title}</p>
                  
                  {isExpanded && (hl.detail || hl.timeRange) && (
                    <div className="text-[12px] text-slate-500 dark:text-slate-400 leading-relaxed mb-2 pb-2 border-b border-slate-50">
                      {hl.timeRange && (
                        <div className="mb-1 font-medium text-slate-600 dark:text-slate-400 flex items-center gap-1.5">
                  ...
                  <button
                    onClick={() => toggleOne(i)}
                    className="text-[11px] text-slate-400 hover:text-slate-600 dark:text-slate-400 font-medium flex items-center gap-1 transition-colors"
                  >
...
function FilterSelectInline({
  ...
  return (
    <div
      ref={ref}
      className="relative h-8 min-w-[130px] flex-1 rounded border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs flex items-center cursor-pointer select-none hover:border-slate-300 dark:border-slate-700 transition"
      onClick={() => setOpen(!open)}
    >
      <div className="flex items-center justify-between gap-1.5 w-full px-2.5">
        <span className="text-[10px] font-medium text-slate-700 dark:text-slate-300 truncate">{selected?.label}</span>
        <ChevronDown className={`h-3 w-3 shrink-0 text-slate-400 transition-transform ${open ? "rotate-180" : ""}`} />
      </div>
      {open && (
        <div className="absolute left-0 top-full z-50 w-full min-w-[150px] mt-0.5 rounded border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 py-1 shadow-lg max-h-60 overflow-y-auto">
          {options.map((opt) => (
            <div
              key={opt.value}
              className={`px-3 py-1.5 text-[11px] cursor-pointer transition-colors hover:bg-slate-50 dark:hover:bg-slate-800 ${
                value === opt.value ? "bg-slate-50 dark:bg-slate-950 font-semibold text-kolia-green" : "text-slate-700 dark:text-slate-300"
              }`}
...
export function YouTubeForeignAnalysis({ domesticPosts = [], variant = "foreign", initialFormat = "" }: { domesticPosts?: any[]; variant?: "foreign" | "domestic"; initialFormat?: string }) {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[380px_1fr] h-[850px]">
      ...
      <div className="flex flex-col h-full rounded border border-kolia-line dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden shadow-xs">
        {/* Left Block Header: Search & Filter */}
        <div className="flex flex-col border-b border-kolia-line dark:border-slate-800 bg-slate-50 dark:bg-slate-950">
          <div className="flex items-center gap-2 p-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 py-1.5 pl-8 pr-3 text-xs text-kolia-ink dark:text-slate-100 placeholder-slate-400 focus:border-kolia-green focus:outline-none"
              />
            </div>
            <button
            ...
            >
              ...
              {(filterTopic || filterChannel || filterFormat) && (
                <span className="ml-0.5 rounded bg-white dark:bg-slate-900/20 px-1 text-[9px] font-bold">{+!!filterTopic + +!!filterChannel + +!!filterFormat}</span>
              )}
            </button>
          </div>
            ...
            <div className="flex flex-wrap items-center gap-2 px-3 pb-3 pt-0">
              ...
              {(filterTopic || filterChannel || filterFormat) && (
                <button
                  ...
                  className="shrink-0 rounded border border-red-200 bg-white dark:bg-slate-900 px-2.5 py-1.5 text-[10px] font-semibold text-red-500 hover:bg-red-50 transition"
                >
                  Xoá
                </button>
              )}
          ...
          {loadingList ? (
            ...
            return (
              <div
                ...
                className={`group flex cursor-pointer gap-3 rounded-lg border p-2.5 bg-white dark:bg-slate-900 transition hover:shadow-sm ${
                  isSelected ? "border-kolia-green bg-emerald-50/20 shadow-sm" : "border-kolia-line dark:border-slate-800 hover:border-slate-300 dark:border-slate-700"
                }`}
              >
                {/* Thumbnail */}
                <div className="relative h-[72px] w-[112px] shrink-0 overflow-hidden rounded-md bg-slate-100 dark:bg-slate-800">
                  <img
                    src={video.thumbnail}
                    alt={video.title}
                ...
                <div className="flex flex-col min-w-0 flex-1 py-0.5 gap-1">
                  {/* Title */}
                  <h3 className="line-clamp-2 text-[11px] font-semibold leading-[1.35] text-kolia-ink dark:text-slate-100 group-hover:text-kolia-green transition">
                    {video.title}
                  </h3>

                  {/* Channel */}
                  <div className="flex items-center gap-1">
                    <span className="truncate text-[10px] text-slate-500 dark:text-slate-400 font-medium">{video.channel}</span>
        ...
        <div className="p-3 border-t border-kolia-line dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex flex-wrap items-center justify-between gap-2 text-[10px] text-slate-500 dark:text-slate-400 select-none">
          <span>
                  ...
                  <button
                    ...
                    className={`h-5 w-5 rounded font-bold text-center text-[11px] leading-none transition ${
                      isCurrent
                        ...
                        : "hover:bg-slate-200/60 text-slate-600 dark:text-slate-400"
                    }`}
                  >
                    {p}
                  </button>
      ...
      <div className="flex flex-col h-full rounded border border-kolia-line dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs overflow-hidden">
        {selectedVideo ? (
          <>
            {/* Header Video Detail */}
            <div className="border-b border-kolia-line dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0">
              <div className="flex gap-4 p-4 items-start">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="relative h-[110px] w-[176px] shrink-0 overflow-hidden rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-800 shadow-sm group block"
                >
                  <img
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-2">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-[15px] font-bold leading-snug text-kolia-ink dark:text-slate-100 line-clamp-2 hover:text-kolia-green hover:underline transition"
                  >
                    {selectedVideo.title}
                  </a>
                  ...
                  <div className="flex items-center gap-1.5">
                    <div className="h-5 w-5 rounded-full bg-slate-200 flex items-center justify-center text-[9px] font-bold text-slate-600 dark:text-slate-400 shrink-0">
                      {selectedVideo.channel.slice(0, 1)}
                    </div>
                    <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{selectedVideo.channel}</span>
                  ...
                  <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      <span className="font-semibold text-slate-700 dark:text-slate-300">{selectedVideo.views}</span>
                      <span>lượt xem</span>
                    </span>
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      <span className="rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-800 px-1.5 py-0.5 text-[9px] font-bold text-slate-600 dark:text-slate-400 whitespace-nowrap">Outlier Score</span>
                    </span>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-white dark:bg-slate-900">
              {loadingAnalysis ? (
                <div className="flex flex-col items-center justify-center py-20 gap-2.5">
                  ...
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">AI đang tiến hành phân tích kịch bản & cấu trúc video...</p>
                </div>
              ) : analysisError ? (
                <div className="flex flex-col items-center justify-center py-20 gap-2.5">
                ...
                <div className="grid grid-cols-2 gap-5">
                  ...
                  <div className="col-span-2 flex items-center justify-between bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg p-3">
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="text-[10px] bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 font-bold border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 px-3 py-1.5 rounded transition"
                      >
                        Phân tích lại
                      </button>
                  ...
                  {/* 1. Format viral */}
                  <section className="rounded border border-kolia-line dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-2xs">
                    <h3 className="text-xs font-bold text-kolia-ink dark:text-slate-100 border-b border-kolia-line dark:border-slate-800 pb-2 mb-3">
                      1. Format viral
                    </h3>
                    <div className="divide-y divide-slate-100 text-xs space-y-2.5">
                      <div className="flex py-1">
                        <span className="w-32 shrink-0 font-semibold text-slate-500 dark:text-slate-400">📋 Mẫu tiêu đề</span>
                        <span className="text-kolia-ink dark:text-slate-100 font-medium">{selectedVideo.formatViral.titleTemplate}</span>
                      </div>
                      <div className="flex py-1">
                        <span className="w-32 shrink-0 font-semibold text-slate-500 dark:text-slate-400">🎯 Hook pattern</span>
                        <span className="text-kolia-ink dark:text-slate-100 font-medium">{selectedVideo.formatViral.hookPattern}</span>
                      </div>
                      <div className="flex py-1">
                        <span className="w-32 shrink-0 font-semibold text-slate-500 dark:text-slate-400">💡 Core promise</span>
                        <span className="text-kolia-ink dark:text-slate-100 font-medium">{selectedVideo.formatViral.corePromise}</span>
                      </div>
                      <div className="flex py-1">
                        <span className="w-32 shrink-0 font-semibold text-slate-500 dark:text-slate-400">🔍 Góc nội dung</span>
                        <span className="text-kolia-ink dark:text-slate-100 font-medium">{selectedVideo.formatViral.contentAngle}</span>
                      </div>
                      <div className="flex py-1">
                        <span className="w-32 shrink-0 font-semibold text-slate-500 dark:text-slate-400">🎙️ Phong cách</span>
                        <span className="text-kolia-ink dark:text-slate-100 font-medium">{selectedVideo.formatViral.style}</span>
                      </div>
                    </div>
                  </section>
                  ...
                  {/* 2. Timeline & cấu trúc */}
                  <section className="rounded border border-kolia-line dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-2xs">
                    <h3 className="text-xs font-bold text-kolia-ink dark:text-slate-100 border-b border-borderColor pb-2 mb-4">
                      2. Timeline & cấu trúc
                    </h3>
                      ...
                      {selectedVideo.segments?.map((seg, i) => (
                        <div key={i} className="flex flex-col flex-1 min-w-[90px] gap-1.5 group snap-start">
                          ...
                          <div className="text-center">
                            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 group-hover:text-kolia-ink dark:text-slate-100 transition-colors whitespace-nowrap">
                              {seg.time}
                            </span>
                          </div>
                          {/* Colored block */}
                        ...
                        if (colonIndex > -1) {
                          ...
                          return (
                            <li key={index} className="flex items-start gap-2">
                              <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-kolia-ink dark:bg-slate-300" />
                              <span>
                                <strong className="text-kolia-ink dark:text-slate-100">{time}:</strong> {desc}
                              </span>
                            </li>
                          );
                        }
                  ...
                  <section className="rounded border border-kolia-line dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-2xs">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {selectedVideo.effects?.map((eff) => (
                        <span
                          key={eff}
                          className="rounded-full bg-emerald-50 dark:bg-emerald-900/30 px-3 py-1 text-[10px] font-bold text-emerald-800 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800"
                        >
                          {eff}
                        </span>
                      ))}
                  ...
                  <section className="rounded border border-kolia-line dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-2xs">
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-slate-800 text-xs space-y-2">
                      <div className="flex py-0.5">
                        <span className="w-28 shrink-0 font-semibold text-slate-500 dark:text-slate-400">Chủ đề</span>
                        <span className="text-kolia-ink dark:text-slate-100 font-semibold">{selectedVideo.brief?.topic}</span>
                      </div>
                      <div className="flex py-0.5">
                        <span className="w-28 shrink-0 font-semibold text-slate-500 dark:text-slate-400">Góc triển khai</span>
                        <span className="text-slate-700 dark:text-slate-300 leading-relaxed">{selectedVideo.brief?.angle}</span>
                      </div>
                      <div className="flex py-0.5">
                        <span className="w-28 shrink-0 font-semibold text-slate-500 dark:text-slate-400">Key points</span>
                        <span className="text-slate-700 dark:text-slate-300 font-medium leading-relaxed">{selectedVideo.brief?.keyPoints}</span>
                      </div>
                      <div className="flex py-0.5">
                        <span className="w-28 shrink-0 font-semibold text-slate-500 dark:text-slate-400">CTA đề xuất</span>
                  ...
                  {variant !== "domestic" && (
                    <section className="col-span-2 rounded border border-kolia-line dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-2xs">
                      <h3 className="text-xs font-bold text-kolia-ink dark:text-slate-100 border-b border-kolia-line dark:border-slate-800 pb-2 mb-3">
                      ...
                      <div className="grid gap-4 xl:grid-cols-[1fr_2fr] items-start">
                        {/* Verdict Card */}
                        <div className="rounded-lg border border-slate-100 dark:border-slate-800/60 bg-slate-50 dark:bg-slate-950 p-3 flex flex-col justify-center min-h-[120px]">
                          ...
                          <div className="mt-2 flex items-start gap-2">
                            ...
                            <div>
                              <h4 className="font-bold text-kolia-ink dark:text-slate-100 text-xs">
                        ...
                        <div className="overflow-x-auto rounded border border-slate-150 dark:border-slate-800">
                          <table className="w-full text-left border-collapse text-[10px]">
                            <thead>
                              <tr className="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800">
                                <th className="p-2 font-bold text-slate-600 dark:text-slate-400">#</th>
                                <th className="p-2 font-bold text-slate-600 dark:text-slate-400">Kênh</th>
                                <th className="p-2 font-bold text-slate-600 dark:text-slate-400">Chủ đề tương tự</th>
                                <th className="p-2 font-bold text-slate-600 dark:text-slate-400 text-right">Lượt xem</th>
                                <th className="p-2 font-bold text-slate-600 dark:text-slate-400 text-right">Độ giống</th>
                                <th className="p-2 font-bold text-slate-600 dark:text-slate-400">Hiệu quả</th>
                                <th className="p-2 font-bold text-slate-600 dark:text-slate-400">Ngày đăng</th>
                              </tr>
                            </thead>
                              ...
                              {selectedVideo.vietnamComparison && selectedVideo.vietnamComparison.table.length > 0 ? (
                                selectedVideo.vietnamComparison.table.map((row: any, i: number) => (
                                  <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800">
                                    <td className="p-2 font-semibold text-slate-500 dark:text-slate-400">{i + 1}</td>
                                    <td className="p-2 font-bold text-kolia-ink dark:text-slate-100">{row.channel}</td>
                                    <td className="p-2 text-slate-600 dark:text-slate-400 leading-relaxed">{row.topic}</td>
                                    <td className="p-2 text-right font-semibold text-slate-700 dark:text-slate-300">{row.views}</td>
                                    <td className="p-2 text-right">
                                      <div className="flex items-center justify-end gap-1">
                                        <span className="h-1.5 w-12 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800 flex">
                                          <span
                                        ...
                                        <span className="font-semibold text-[9px] text-slate-600 dark:text-slate-400">{row.relevanceScore}%</span>
                                      </div>
                                    </td>
                                    <td className="p-2">
                                      <div className="flex items-center gap-1">
                                        <span className="h-1 w-10 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800 flex">
                                          <span
                                            className={`h-full rounded-full ${
                                              row.efficiency === "Cao"
                                        ...
                                        <span className="font-semibold text-[9px] text-slate-600 dark:text-slate-400">{row.efficiency}</span>
                                      </div>
                                    </td>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedVideo.tags.map((tag) => (
                    <span
                      key={tag}
                      className="rounded bg-slate-100 dark:bg-slate-800 px-2.5 py-1 text-[10px] font-semibold text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-800"
                    >
                      #{tag}
                    </span>
                  ))}
                ...
                selectedVideo.channelProfile ? (
                  <div className="space-y-4">
                    {/* Header */}
                    <div className="flex items-center gap-3 rounded-lg border border-slate-150 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-2xs">
                      <div className="h-12 w-12 rounded-full bg-gradient-to-br from-slate-200 to-slate-300 flex items-center justify-center font-bold text-slate-600 dark:text-slate-400 text-lg uppercase shadow-inner shrink-0">
                        {selectedVideo.channel.slice(0, 2)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-sm text-kolia-ink dark:text-slate-100">{selectedVideo.channel}</h4>
                    ...
                    {/* Details */}
                    <div className="rounded-lg border border-slate-150 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-2xs space-y-3 text-xs">
                      <div className="flex gap-3 py-2 border-b border-slate-100 dark:border-slate-800/60">
                        <span className="w-36 shrink-0 font-semibold text-slate-500 dark:text-slate-400">🎬 Phong cách ND</span>
                        <span className="text-slate-700 dark:text-slate-300 leading-relaxed">{selectedVideo.channelProfile.contentStyle}</span>
                      </div>
                      <div className="flex gap-3 py-2 border-b border-slate-100 dark:border-slate-800/60">
                        <span className="w-36 shrink-0 font-semibold text-slate-500 dark:text-slate-400">👥 Đối tượng</span>
                        <span className="text-slate-700 dark:text-slate-300 leading-relaxed">{selectedVideo.channelProfile.targetAudience}</span>
                      </div>
                      <div className="flex gap-3 py-2 border-b border-slate-100 dark:border-slate-800/60">
                        <span className="w-36 shrink-0 font-semibold text-slate-500 dark:text-slate-400">💪 Điểm mạnh</span>
                        <span className="text-slate-700 dark:text-slate-300 leading-relaxed">{selectedVideo.channelProfile.strengths}</span>
                      </div>
                      <div className="flex gap-3 py-2">
                        <span className="w-36 shrink-0 font-semibold text-slate-500 dark:text-slate-400">💡 Nhận xét</span>
                        <span className="text-slate-700 dark:text-slate-300 leading-relaxed">{selectedVideo.channelProfile.reason}</span>
                      </div>
                    </div>
                  </div>
                ) : (
          ...
          <div className="flex items-center justify-center h-full text-slate-400 text-xs bg-slate-50 dark:bg-slate-900/10">
      ...
      {showMoreVietnamVideos && vietnamVideosFullData && (
        ...
        >
          <div
            className="bg-white dark:bg-slate-900 sm:rounded-2xl shadow-2xl w-full h-full sm:h-[82vh] sm:max-w-6xl flex flex-col overflow-hidden"
          ...
          >
            ...
            <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 shrink-0">
              <div className="min-w-0">
                <h3 className="text-[15px] font-bold text-slate-800 dark:text-slate-200 truncate">
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
              ...
              <div className="lg:w-1/4 border-t lg:border-t-0 lg:border-l border-slate-200 dark:border-slate-800 flex flex-col min-h-0">
                {/* Verdict summary */}
                <div className="shrink-0 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-3">
                  <div className="flex items-start gap-2">
                    <div className="rounded-full bg-emerald-100 p-0.5 text-kolia-green shrink-0 mt-0.5">
                      <Check className="h-3 w-3 stroke-[3]" />
                    </div>
                    <div>
                      <h4 className="font-bold text-kolia-ink dark:text-slate-100 text-[11px]">{vietnamVideosFullData.quickVerdict}</h4>
                      <p className="mt-0.5 text-[9px] text-slate-500 dark:text-slate-400 leading-relaxed">{vietnamVideosFullData.quickVerdictDesc}</p>
                    </div>
                  </div>
                </div>
                    ...
                    return (
                      <div
                        ...
                        className={`group flex gap-2.5 rounded-xl p-2 cursor-pointer transition-colors ${
                          isSelected
                            ? "bg-kolia-ink text-white"
                            : "hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800"
                        }`}
                      >
                        {/* Thumbnail */}
                        <div className="relative h-[52px] w-[80px] shrink-0 overflow-hidden rounded-lg bg-slate-100 dark:bg-slate-800">
                          {row.thumbnailUrl ? (
                            <img
                          ...
                          <span className={`absolute bottom-0.5 left-0.5 flex h-[16px] w-[16px] items-center justify-center rounded-full text-[8px] font-bold leading-none ${
                            isSelected ? "bg-white dark:bg-slate-900/25 text-white" : "bg-black/75 text-white"
                          }`}>
                            {i + 1}
                          </span>
                        </div>
                        ...
                        {row.postUrl && (
                          <a
                            ...
                            className={`shrink-0 opacity-0 group-hover:opacity-100 transition self-start mt-1 ${
                              isSelected ? "text-white/60 hover:text-white" : "text-slate-400 hover:text-slate-600 dark:text-slate-400"
                            }`}
                          >
                            <ExternalLink className="h-3 w-3" />
                          </a>
                ...
                {vietnamVideosFullData.hasMore && (
                  <div className="shrink-0 border-t border-slate-100 dark:border-slate-800/60 p-2 text-center">
