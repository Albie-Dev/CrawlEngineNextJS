...
function ViralLoadingProgress() {
  ...
  return (
    <div className="mx-auto max-w-lg py-16">
      <div className="rounded-xl border border-kolia-line dark:border-slate-800 dark:border-slate-800 bg-white dark:bg-slate-900 dark:bg-slate-900 p-8 shadow-sm">
              ...
              <div key={i} className={`flex items-center gap-4 transition-opacity ${isActive ? "opacity-100" : isDone ? "opacity-60" : "opacity-30"}`}>
                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 transition-all ${
                  isDone
                    ...
                    : "border-slate-200 dark:border-slate-800 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 dark:bg-slate-900/50 text-slate-400"
                }`}>
                  {isDone ? (
                ...
                <div className="flex-1">
                  <p className={`text-sm font-semibold ${isDone ? "text-green-700" : isActive ? "text-kolia-ink dark:text-slate-100 dark:text-slate-100" : "text-slate-400"}`}>
                    {step.label}
                  </p>
                  {isActive && (
                    <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800 dark:bg-slate-800">
  ...
  return (
    <div className="space-y-6">
      ...
      {isPending ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-kolia-line dark:border-slate-800 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 dark:bg-slate-900/50 py-20 text-center">
          <BrainCircuit className="mb-4 h-12 w-12 text-slate-300" />
          <p className="font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-400">Chọn khoảng thời gian và nhấn <span className="text-kolia-green">Phân tích</span></p>
        ...
        <>
          ...
          {data.patterns.length > 0 && (
            <section className="rounded border border-kolia-line dark:border-slate-800 dark:border-slate-800 bg-white dark:bg-slate-900 dark:bg-slate-900 p-5 shadow-sm">
              <h2 className="font-bold text-kolia-ink dark:text-slate-100 dark:text-slate-100">🔥 Viral Patterns</h2>
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                {data.patterns.map((p) => {
                  const Icon = typeIcons[p.type] ?? TrendingUp;
                  return (
                    <div key={p.id} className="rounded border border-kolia-line dark:border-slate-800 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 dark:bg-slate-900/50 p-4">
                      ...
                      <h3 className="mt-2 font-semibold text-kolia-ink dark:text-slate-100 dark:text-slate-100">{p.name}</h3>
                      <p className="mt-1 text-sm text-slate-600 dark:text-slate-400 dark:text-slate-400">{p.description}</p>
                      <div className="mt-2 flex flex-wrap gap-3 text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">
                      ...
                      {p.examplePost && (
                        <div className="mt-2 rounded bg-white dark:bg-slate-900 dark:bg-slate-900 p-2 text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">
          ...
          {data.emergingTrends.length > 0 && (
            <section className="rounded border border-kolia-line dark:border-slate-800 dark:border-slate-800 bg-white dark:bg-slate-900 dark:bg-slate-900 p-5 shadow-sm">
              <h2 className="font-bold text-kolia-ink dark:text-slate-100 dark:text-slate-100">📈 Emerging Trends</h2>
              <div className="mt-4 space-y-3">
                {data.emergingTrends.map((t, i) => (
                  <div key={i} className="flex items-center justify-between rounded border border-kolia-line dark:border-slate-800 dark:border-slate-800 p-4">
                    <div>
                      <p className="font-semibold text-kolia-ink dark:text-slate-100 dark:text-slate-100">{t.topic}</p>
                      <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400">{t.description}</p>
                    </div>
          ...
          {data.clusters.length > 0 && (
            <section className="rounded border border-kolia-line dark:border-slate-800 dark:border-slate-800 bg-white dark:bg-slate-900 dark:bg-slate-900 p-5 shadow-sm">
              <h2 className="font-bold text-kolia-ink dark:text-slate-100 dark:text-slate-100">📊 Content Clusters</h2>
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                {data.clusters.map((c) => (
                  <div key={c.id} className="rounded border border-kolia-line dark:border-slate-800 dark:border-slate-800 p-4">
                    <h3 className="font-semibold text-kolia-ink dark:text-slate-100 dark:text-slate-100">{c.label}</h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">{c.size} bài · engagement BQ {(c.avgEngagement * 100).toFixed(1)}%</p>
                    {c.commonPatterns.length > 0 && (
                      <ul className="mt-2 space-y-1 text-xs text-slate-600 dark:text-slate-400 dark:text-slate-400">
