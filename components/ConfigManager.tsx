  ...
  return (
    <div className="space-y-6">
      ...
      {Object.entries(grouped).map(([category, items]) => (
        <section key={category} className="rounded border border-kolia-line dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm">
          <h2 className="font-bold text-kolia-ink dark:text-slate-100">{CATEGORY_LABELS[category] || category}</h2>
          {category === "openai" && (
            <p className="mt-1 text-xs text-slate-400">
              Provider hiện tại: <strong className="text-kolia-ink dark:text-slate-100">{activeAiProvider}</strong>
            ...
            {items.map((item) => {
              ...
              return (
                <div key={item.key} className={`rounded border p-4 transition ${
                  item.hasValue ? "border-kolia-line dark:border-slate-800 bg-slate-50 dark:bg-slate-950" : "border-amber-200 bg-amber-50"
                }`}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <h3 className="font-semibold text-kolia-ink dark:text-slate-100">{item.label}</h3>
                        {item.hasValue ? (
                      ...
                      {isEditing ? (
                        <div className="mt-2 flex items-center gap-2">
                          ...
                          {item.isSecret && item.key !== "ai_provider" && (
                            <button onClick={() => setShowSecret(isVisible ? null : item.key)} className="text-slate-400 hover:text-slate-600 dark:text-slate-400">
                          ...
                          <button onClick={() => setEditingKey(null)} className="rounded border px-3 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800">
                            <X className="h-3 w-3" />
                          </button>
                        </div>
                      ) : (
                      ...
                      <div className="flex shrink-0 gap-2">
                        {CONFIG_HELP[item.key] && (
                          <button
                            ...
                            className="rounded border px-2 py-1.5 text-xs text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-kolia-green"
                            title="Xem hướng dẫn"
                          >
                        ...
                        {item.source !== "env" && (
                          <button
                            ...
                            className="rounded border px-2.5 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800"
                          >
                            Sửa
                          </button>
                        )}
                  ...
                  {helpKey === item.key && CONFIG_HELP[item.key] && (
                    <div className="mt-3 rounded-lg border border-kolia-line dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
      ...
      {deleteKey && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-xl bg-white dark:bg-slate-900 p-6 shadow-2xl">
            <h3 className="text-lg font-bold text-kolia-ink dark:text-slate-100">🗑️ Xác nhận xoá</h3>
            <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">
              Xoá config này? Sẽ dùng env fallback nếu có.
            </p>
            <div className="mt-6 flex items-center justify-end gap-3">
              <button onClick={() => setDeleteKey(null)} className="rounded-lg px-4 py-2 text-sm font-semibold text-slate-600 dark:text-slate-400 transition hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800">
                Giữ lại
              </button>
      ...
      {/* Info */}
      <div className="rounded border border-kolia-line dark:border-slate-800 bg-kolia-amber p-4 text-sm leading-6 text-slate-600 dark:text-slate-400">
