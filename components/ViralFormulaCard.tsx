      ...
      {formula.timeline && formula.timeline.length > 0 && (
        <div className="mt-4 space-y-3">
          {formula.timeline.map((step, i) => (
            <div key={i} className="rounded border border-kolia-line bg-slate-50 p-3">
              <div className="flex flex-wrap items-center gap-2">
                {step.time && <span className="rounded bg-white px-2 py-1 text-xs font-bold text-kolia-gold">{step.time}</span>}
              ...
              {step.role && <p className="mt-2 text-xs font-semibold leading-5 text-slate-500">Vai trò: {step.role}</p>}
            </div>
          ))}
        </div>
      )}
